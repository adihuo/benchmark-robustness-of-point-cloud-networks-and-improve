...
class LPFA(nn.Module):
    def __init__(self, in_channel, out_channel, k, mlp_num=2, initial=False):
        ...
        self.xyz2feature = nn.Sequential(
            nn.Conv2d(3, in_channel, kernel_size=1, bias=False),
    ...
    def group_feature(self, x, xyz, idx):
        ...
        if self.initial:
            feature = self.xyz2feature(feature)  # bs, c, n, k
...
class CurveNet_AA(nn.Module):
    def __init__(self, num_classes=15, k=20, setting='default'):
        super(CurveNet_AA, self).__init__()
