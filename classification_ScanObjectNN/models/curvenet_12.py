...
class CIC(nn.Module):
    def __init__(self, npoint, radius, k, in_channels, output_channels, bottleneck_ratio=2, mlp_num=2,
        ...
        self.gettingmodule = GettingModule(output_channels)
...
class CurveAggregation(nn.Module):
    ...
    def forward(self, x, curves):
        curves_att = self.line_conv_att(curves)  # curves: bs, c, c_n ,c_l   curves_att: bs, 1, c_n, c_l
...
class GettingModule(nn.Module):
    def __init__(self, in_channel, scaling=4):
        super(GettingModule, self).__init__()

        self.fc1 = nn.Conv1d(in_channel, in_channel // scaling, kernel_size=1, bias=False)
        self.relu1 = nn.ReLU()
        self.fc2 = nn.Conv1d(in_channel // scaling, in_channel, kernel_size=1, bias=False)
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(in_channel,
                      ...
                      kernel_size=1,
                      bias=False),
            nn.BatchNorm1d(in_channel // scaling),
            nn.LeakyReLU(negative_slope=0.2, inplace=True))
        self.conv2 = nn.Sequential(
            nn.Conv1d(in_channel // scaling,
                      in_channel,
                      kernel_size=1,
                      bias=False),
            nn.BatchNorm1d(in_channel),
    ...
    def forward(self, x):
        max_out = x * x.max(dim=1, keepdim=True)[0]
        avg_out = x * x.mean(dim=1, keepdim=True)[0]
        # x1 = max * x
        # x2 = avg * x
        # 池化后维度不对应
        # x = self.fc1(x)
        # x = self.relu1(x)
        max_out = self.fc2(self.conv1(max_out))
        avg_out = self.fc2(self.conv1(avg_out))
...
class CurveNet_12(nn.Module):
    def __init__(self, num_classes=15, k=20, setting='default'):
        super(CurveNet_12, self).__init__()
