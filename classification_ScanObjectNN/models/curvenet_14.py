...
class GettingModule(nn.Module):
    ...
    def forward(self, x):
        max_out = x * F.softmax(x.max(dim=1, keepdim=True)[0], dim=-1)
        avg_out = x * F.softmax(x.mean(dim=1, keepdim=True)[0], dim=-1)
...
class CurveNet_14(nn.Module):
    def __init__(self, num_classes=15, k=20, setting='default'):
        super(CurveNet_14, self).__init__()
