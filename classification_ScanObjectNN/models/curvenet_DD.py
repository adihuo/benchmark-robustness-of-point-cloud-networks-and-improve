...
class LPFA(nn.Module):
    ...
    def group_feature(self, x, xyz, idx):
        ...
        a = points
        b = point_feature
        c = torch.cat((points, point_feature, point_feature - points),dim=3)
...
class CurveAggregation(nn.Module):
    ...
    def forward(self, x, curves):
        ...
        curves_att = curves_att.view(bt, 1, -1)
        curves = curves.view(bt, ch, -1)

        curves_all = curves * F.softmax(curves_att, dim=-1)
        ...
        curves_all = self.convb(curves_all)  # bs, mid ,n
        ...
        x_all = F.softmax(torch.bmm(x_logits, curves_all), dim=-1)  # bs, n, c_all
...
class CurveNet_DD(nn.Module):
    def __init__(self, num_classes=15, k=20, setting='default'):
        super(CurveNet_DD, self).__init__()
