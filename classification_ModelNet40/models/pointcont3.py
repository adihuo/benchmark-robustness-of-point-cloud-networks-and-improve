...
def save_tensor_to_txt(tensor, file_path):
    """
    Save a PyTorch tensor to a text file.
    ...
    if numpy_array.ndim > 2:
        flattened_array = numpy_array.reshape(-1, numpy_array.shape[-1])
    else:
        flattened_array = numpy_array

    # Save NumPy array to text file
    np.savetxt(file_path, flattened_array)
...
class ConT(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = qkv.reshape(B, S, 3, self.num_heads, D // self.num_heads).permute(2, 0, 3, 1, 4)  # [3, B, h, S, d]
...
class Backbone(nn.Module):
    def __init__(self, patch_dim=[3, 64, 128, 256, 512, 1024], dropout=0.5, num_classes=40, num_points=1024,
        ...
        for i in range(self.nblocks):
            ...
            self.patch_embedding.append(MLPBlock1D(patch_dim[i + 1], patch_dim[i + 1]))
    ...
    def forward(self, x):
        ...
        for i in range(self.nblocks):
            ...
            features = self.patch_embedding[i](max_features.transpose(1, 2)).transpose(1, 2)
            pos_and_feats.append([pos, features])
...
if __name__ == '__main__':
    def generate_random_tensor(shape, low=0, high=3000):
        """
        Generate a random torch tensor with elements in the specified range.

        Parameters:
            shape (tuple): Desired shape of the tensor.
            low (int): Lower bound of the random values (default: 0).
            high (int): Upper bound of the random values (default: 3000).

        Returns:
            torch.Tensor: Random tensor with elements in the specified range.
        """
        # Generate random integers in the specified range
        random_integers = torch.randint(low, high, size=shape, dtype=torch.float32)

        # Map the integers to the desired range [low, high]
        random_tensor = (random_integers / 3000) * (high - low) + low

        return random_tensor


    # Example usage
    shape = (20, 1024, 3)  # Specify the desired shape of the tensor
    random_generated_tensor = generate_random_tensor(shape)
    data = random_generated_tensor.cuda()
